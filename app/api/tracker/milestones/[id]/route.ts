...
import { updateTrackerMilestone, deleteTrackerMilestone } from '@/lib/tracker'
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const ms = await updateTrackerMilestone(Number(params.id), b)
...
export async function DELETE(_: NextRequest, { params }: { params: { id: string } }) {
  const { err } = await adminOnly()
  if (err) return err
  await deleteTrackerMilestone(Number(params.id))
